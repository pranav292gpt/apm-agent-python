...
1.  Choose a service name, and set the secret token if needed.Set the server address of the APM server instance. (If SERVER_URL is not set, by default it will be set to 'localhost:8200'.)
